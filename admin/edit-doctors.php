...
if(isset($_POST['update_doctors'])) {
	...
	try {
		...
		$statement = $db->prepare("UPDATE doctors SET  doc_pic=?, name=?,  sex=?, birthday=?, designation=?, speciality=?, consulting_hour=?, consulting_day=?, room_no=?, contact=?, address=?, email=? WHERE doc_id=?");
		$statement->execute(array($f1, $_POST['name'],$_POST['sex'],$birthday,$_POST['designation'],$_POST['speciality'],$_POST['consulting_hour'],$_POST['consulting_day'],$_POST['room_no'],$_POST['contact'],$_POST['address'],$_POST['email'],$doc_id));
...
<?php include('header.php');?>
	...
	<?php
					...
					$statement = $db->prepare("SELECT * FROM doctors WHERE doc_id=? ORDER By doc_id");
