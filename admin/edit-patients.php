...
if(isset($_POST['add_patient'])) {
	...
	try {
		...
		$statement = $db->prepare("UPDATE patients SET p_pic=?, p_name=?,sex=?,age=?,blood_group=?,type_of_disease=?,treating_doctor=?,room_no=?,contact=?,p_status=?,total_bill=?,address=?,email=?,add_date=? WHERE p_id=?");
		$statement->execute(array($f1,$_POST['name'],$_POST['sex'],$_POST['age'],$_POST['blood_group'],$_POST['disease'],$_POST['treating_doctor'],$_POST['room_no'],$_POST['contact'],$_POST['p_status'],$_POST['total_bill'],$_POST['address'],$_POST['email'],$_POST['add_date'],$p_id));
...
<?php include('header.php');?>
	...
	<?php
					...
					$statement = $db->prepare("SELECT * FROM patients WHERE p_id=? ORDER By p_id");
